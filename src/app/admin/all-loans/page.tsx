...
export default function AdminGroupLoansPage() {
  const [loans, setLoans] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [expandedLoanId, setExpandedLoanId] = useState<string | null>(null);

  useEffect(() => {
    // Reusing the member endpoint which fetches all group loans safely for session users
    // Alternatively, we can use the admin/loans GET endpoint which returns the same data
